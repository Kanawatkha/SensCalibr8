...
- Regression scope: exercised the normal Winner path, statistical tie, eDPI floor collision/deduplication, incomplete batteries, undefined CV, stabilization minimum/maximum, outlier/data-quality separation, fatigue, Grade boundaries, plateau/non-plateau, cancellation, restart, and historical version preservation across the existing Phase 1-3, engine, persistence, scoring, and continuous-cycle tests.
- Test results: Unity EditMode **213/213 passed**, with 0 failed, 0 skipped, and 0 inconclusive. Windows production build passed (SensCalibr8.exe, 667648 bytes); calibration analysis regression **72/72 passed**; git diff --check passed.
- Result: **P5-R8 complete.** Phase 5 is complete and no Git operation was performed.

### Phase 5 / P5-R7: Continuous cycle and plateau

- Contract and training: added immutable `sc8-continuous-cycle-v1`, defining the source-authorized 5-10 Database Session training block, training purpose, Phase 3 Winner as Best/baseline source, three-cycle window, strictly-below-5% relative score criterion, and no-plateau handling for a near-zero baseline. The training workflow schedules mode sessions deterministically/counterbalanced in four-mode blocks while retaining Database Session accounting.
- Checkpoint: added a fail-closed checkpoint boundary which requires the Phase 3 Winner to map to one canonical candidate, 5-10 persisted training sessions, and the latest complete scored/graded training battery. It stores the Best eDPI, actual session/battery counts, score, Grade, contract version, and source Phase 3 history identity; partial batteries cannot fabricate a Grade checkpoint.
- Plateau and recalibration: evaluates only the latest three checkpoints. All three Grades must be exactly equal and the absolute first-to-third score change must be strictly below 5%; exactly 5% and a near-zero baseline do not pass. A passing plateau creates exactly one successor cycle transactionally, preserving a recalibration event and a fresh Phase 1 seven-value candidate set around the source Phase 3 Winner eDPI. It never mutates `profiles.current_sensitivity`.
- Persistence: migration 8 adds unique `(profile_id, cycle_number)` identity, `cycle_checkpoints`, and `recalibration_events`; all eight new foreign keys use `ON DELETE CASCADE`. One source cycle/checkpoint can create only one recalibration successor.
- Test results: Unity EditMode **213/213 passed**, including contract values, the five-session guard, 4% same-Grade plateau/recalibration, exact-5% non-plateau, successor cycle/candidate/provenance checks, duplicate-recalibration rejection, migration version/index/FK counts, and prior regressions. Windows production build passed (`SensCalibr8.exe`, 667648 bytes); calibration analysis regression **72/72 passed**; `git diff --check` passed.
- Result: **P5-R7 complete**. No full end-to-end regression sweep and no Git operation were performed.

### Phase 5 / P5-R6: Adaptation, outlier, fatigue, and Grade

- Versioned contract: added immutable `sc8-scientific-rigor-v1`. The loader fails closed unless its 50% shot adaptation and first Tracking block match frozen mode configuration and its 3-SD multiplier matches centralized research constants. Fatigue percentage scaling/strict 15% threshold and Grade identities are loaded rather than embedded in logic.
- Adaptation and scoring: retained the post-session adaptation boundary established in P3-R5. Scientific analysis accepts only finalized post-adaptation observations. Scoring now supports validated observation subsets for chronological fatigue halves and flagged-row-excluded sensitivity analysis while preserving exact-count authoritative scoring.
- Outlier control: implemented one-pass sample-SD analysis with strict absolute deviation `> 3 x SD`, canonical homogeneous `(profile, cycle, phase, mode, sensitivity, metric)` scope identity, raw-row lineage checks, denormalized shot flags, and immutable run-level statistics/version audit. Statistical flags remain in the authoritative score; a parallel excluded sensitivity score is available for reporting. An authoritative exclusion requires a second explicit call with a non-empty documented acquisition/data-quality reason.
- Fatigue: implemented chronological `floor(n/2)` first half plus remaining second half after adaptation, separate subset Performance Scores, strict decline greater than 15%, near-zero denominator handling under the frozen scoring tolerance, and one-time session persistence of percentage/flag/algorithm version. Fatigue remains informational and does not enter Winner filtering.
- Grade: implemented exact Reaction boundaries at 200/250/350/500 ms, frozen Consistency utility bands at 0.8/0.6/0.4/0.2, authoritative Close Flick Reaction input, four-mode mean Consistency input, and worse-tier final Grade. Component tiers, source values, and `sc8-grade-v1` are persisted once and cannot be overwritten.
- Persistence: migration 7 adds `outlier_analysis_runs`, outlier-run linkage, fatigue algorithm lineage, and Grade component/version audit fields. All four new foreign keys use `ON DELETE CASCADE`; duplicate outlier scope, fatigue assignment, and Grade assignment fail closed.
- Test results: Unity EditMode **209/209 passed**, including contract drift guards, one-pass strict 3-SD flagging, raw-evidence value matching, inclusive/excluded sensitivity scores, documented data-quality exclusion, adaptation-scope rejection, exact fatigue threshold and half scoring, all Reaction/Consistency Grade boundaries, one-time persistence, migration version/index/FK counts, and prior regressions. Windows production build passed (`SensCalibr8.exe`, 667648 bytes); calibration analysis regression **72/72 passed**; no-hardcoded-threshold scan and `git diff --check` passed.
- Result: **P5-R6 complete**. No continuous-cycle/plateau behavior and no Git operation were performed.

### Phase 5 / P5-R5: Phase 3 final narrowing

- Decision and winner rule: the project owner approved OQ-022 on 2026-07-17. Only after every candidate has stabilized, the unique highest mean complete-battery Performance Score becomes the phase Winner. Exactly equal highest means are an explicit tie: no arbitrary eDPI tie-break, no `phase_history` row, and no subsequent automatic narrowing/final-Best action.
- Phase 2 finalization: added an atomic phase-history boundary which verifies profile/cycle lineage and stabilization for every Phase 2 candidate before persisting the unique Winner. Duplicate winner history for a profile/cycle/phase is rejected.
- Phase 3 protocol: added accepted immutable `sc8-phase-three-protocol-v1`. It creates `-5/0/+5%` candidates around the persisted Phase 2 Winner, applies the existing eDPI floor before canonical deduplication, and preserves all source paths when values collapse. It reuses the accepted Phase 2 5-10 complete-battery, strict-CV-below-10 contract as required by the documented “same test structure” rule.
- Blind execution and finalization: Phase 3 uses deterministic opaque labels and four-mode counterbalancing, constrains repetitions to the accepted 1-10 interval, and prevents duplicate candidate/repetition launch. Its final Winner uses the approved highest-mean/exact-tie rule and is persisted as the preliminary Best Sensitivity in Phase 3 history. `profiles.current_sensitivity` is deliberately not changed automatically; the user-owned live-game setting remains explicit.
- Test results: Unity EditMode **190/190 passed**, including Phase 2 high-mean persistence, exact-equal tie with no history, Phase 3 `266/280/294` eDPI generation from `280`, blind launch, final Winner persistence, floor/dedup provenance, and non-stabilized rejection. Windows production build passed (`SensCalibr8.exe`, 667648 bytes); calibration analysis regression **72/72 passed**; `git diff --check` passed.
- Result: **P5-R5 complete**. No Git operation was performed.

### Phase 5 / P5-R4: Phase 2 progressive narrowing

- Configuration: added accepted immutable `sc8-phase-two-protocol-v1` containing the source-authorized -10/0/+10 offsets, 5/10 complete-battery bounds, strict CV-below-10 threshold, narrowing purpose, and single/tie generation identities. Logic contains no independent protocol thresholds.
- Candidate generation: consumes exactly one persisted Phase 1 significance decision. Candidate A/B winners generate one anchor at -10/0/+10; statistical ties generate the union of both anchors. Values are not intermediately rounded, eDPI 160 floor adjustments are notified, canonical eDPI is deduplicated after flooring, and every anchor/offset/pre-floor/floor-applied source is inserted atomically.
- Blind execution: added deterministic blind Phase 2 candidate ordering and four-mode counterbalancing. Battery repetition is constrained to the accepted 1-10 range, batteries use purpose `narrowing`, numeric sensitivity/eDPI remain off the public launch surface, duplicate candidate/repetition launches are rejected, and stabilized/maximum-failed candidates cannot launch more evidence.
- Stabilization: reads only complete, battery-linked narrowing scores with matching candidate/configuration/formula lineage. It reports `Collecting` before five, `Stabilized` only for sample-SD CV strictly below 10%, `RequiresMoreEvidence` through nine for CV >=10 or undefined near-zero mean, and `MaximumReachedWithoutStabilization` at ten. Phase readiness requires every candidate to stabilize; incomplete batteries are ignored.
- Test results: Unity EditMode **184/184 passed**, including immutable contract identity, exact winner set 252/280/308 eDPI at DPI 1600, tie floor/dedup with all six sources, blind narrowing battery/counterbalance, exact worked CV **5%**, strict CV **10%** non-pass, undefined zero mean, incomplete-battery exclusion, and maximum failure at 10. Windows production build passed (`SensCalibr8.exe`, 667648 bytes); calibration analysis regression **72/72 passed**; `git diff --check` passed.
- Specification issue: no authoritative document defines how to choose one Phase 2 Winner after multiple candidates have individually stabilized. This is recorded as OQ-022; no highest-mean, tie-break, or additional significance rule was invented.
- Result: **P5-R4 complete for generation, execution, and stabilization**. Phase 2 Winner finalization and P5-R5 remain blocked by OQ-022. No Git operation was performed.

### Phase 5 / P5-R3: Confirmatory significance gate

- Configuration: added an immutable typed projection of `sc8-confirmatory-v1` from the SHA-verified frozen configuration. Production consumes the accepted pair count, exhaustive enumeration count, test method, alternative, alpha, confidence level, df9 t critical, and inclusive-extremeness tolerance; exploratory reuse and early stopping are rejected.
- Ranking and execution: added a blind production workflow that ranks completed battery-linked exploratory scores, selects the top two candidates as A/B, creates each confirmatory battery pair atomically, follows the frozen five-A-first/five-B-first order, and exposes pairing seed plus matched-condition key without exposing eDPI, sensitivity, or exploratory score.
- Statistics: implemented the exact two-sided paired sign-flip randomization test over all 1024 assignments, paired mean A-minus-B effect, and two-sided 95% Student-t interval. Strict `p < 0.05` selects the effect-sign candidate; otherwise the result is `statistical_tie`.
- Persistence/integrity: added migration 6 linking every new `sensitivity_tests` aggregate to one unique parent battery. Score persistence now requires a complete four-distinct-mode battery with matching profile/cycle/phase/eDPI/configuration. Significance results and all 10 pair rows commit in one transaction only after every battery is completed, purpose-`confirmatory`, fresh, non-reused, candidate-matched, and exactly matched to its persisted battery score/formula/configuration lineage.
- Test results: Unity EditMode **176/176 passed**, including the accepted positive fixture effect **5**, p **0.001953125**, CI **[5,5]**, candidate A; symmetric tie; candidate B; 5/5 order; deterministic matched keys; missing/duplicate pairs; incomplete battery; evidence reuse; score tampering; blind DTO surface; migration version/index/FK counts; and no-partial-result assertions. Windows production build passed (`SensCalibr8.exe`, 667648 bytes); calibration analysis regression **72/72 passed**; `git diff --check` passed.
- Issues resolved: corrected an initial lineage query that assumed a physical `formula_version` column on `calibration_configs`; formula identity is instead read from the accepted immutable configuration and stored on score/significance rows. Added the previously missing score-to-battery relationship so confirmatory inputs cannot be untraceable numeric values.
- Result: **P5-R3 complete**. No Phase 2 candidate generation/CV stabilization, Phase 3 narrowing, Grade, outlier, fatigue, plateau, or Git operation was performed.

### Phase 5 / P5-R2: Phase 1 exploratory protocol

- Configuration: added accepted immutable `sc8-protocol-constants-v1` for the seven Phase 1 offsets and schema-compatible generation lineage. Candidate calculations use the PSA baseline, retain full floating-point values without intermediate rounding, and apply the accepted eDPI floor before sensitivity conversion.
- Floor/provenance behavior: every below-floor source produces a user-facing adjustment notification. Sources that converge on the same final eDPI are deduplicated into one canonical candidate while preserving all anchor, offset, pre-floor eDPI, and floor-applied rows.
- Persistence: candidate sets and all source rows are inserted in one immediate SQLite transaction. A failure in any candidate/source rolls back the entire set. `ProtocolPhase` identities are explicitly 1/2/3 to match the persistent schema rather than relying on zero-based enum ordinals.
- Blind execution: `PhaseOneExploratoryWorkflow` exposes ordered opaque `Candidate-NN` labels, never eDPI or sensitivity, and creates exploratory batteries with deterministic four-mode counterbalancing across repetitions. Duplicate launch of the same candidate repetition is rejected.
- Completion contract: `PhaseOneSampleContract` projects counts from the accepted frozen configuration. Each shot mode requires 30 total / 15 adaptation / 15 authoritative observations; Tracking requires 18 total / 9 adaptation trials and 108 total / 54 authoritative windows.
- Test results: Unity EditMode **168/168 passed**, including exact DPI 1600 worked values, no-rounding generation, floor deduplication/provenance, transactional rollback, blind-surface reflection checks, four-repetition mode counterbalancing, and frozen sample gates. Windows production build passed (`SensCalibr8.exe`, 667648 bytes); calibration analysis regression **72/72 passed**; `git diff --check` passed.
- Issues resolved: acceptance tests caught a zero-based `ProtocolPhase`/positive-schema mismatch and an unsupported candidate `generation_rule`; both contracts now align with the existing database constraints.
- Result: **P5-R2 complete**. No stabilization narrowing, ranking, Grade, significance test, Winner selection, or Git operation was performed.

### Phase 5 / P5-R1: Metric normalization and score services

- Configuration: extended the accepted immutable configuration projection with a typed `ScoringFormulaContract` built from the SHA-verified top-level `formula_contract`. Formula weights, multiplier, and authoritative sample counts are therefore loaded from `calibration_config_v1`; production scoring does not duplicate them as constants.
- Calculation: added plain-C# fixed min-max high/low utility normalization with clamping, sample-SD Consistency, hit-only Submovement averaging with the accepted 1-6 bounds, zero-hit fail-closed penalty, shot-mode formulas without a final clamp, Tracking redistributed weights, exact 15-shot/54-window aggregation, and unweighted four-mode battery averaging.
- Missing data: all required authoritative values fail closed. The only implemented scoring fallbacks are the approved Far missing-onset ceiling and zero-hit Submovement penalty; raw nullable data remains unchanged.
- Persistence: added typed `SensitivityTestRepository` and `SensitivityScorePersistenceService`. Every stored aggregate carries the active `formula_version`, references the accepted calibration configuration (which identifies normalization), and preserves the four mode scores as JSON.
- Test results: Unity EditMode **161/161 passed**, including exact worked examples Shot **77.0** and Tracking **81.875**, normalization/clamp boundaries, theoretical shot floor **-10**, Submovement 1/6 bounds, 15/54 sample gates, missing-data rejection, four-mode uniqueness, and SQLite formula-lineage persistence. Windows production build passed (`SensCalibr8.exe`, 667648 bytes); calibration analysis regression **72/72 passed**; `git diff --check` passed. Production Python completed **10/11** checks and was blocked only by missing installed-package metadata for `contourpy` in the bundled runtime, the same environment-only condition previously recorded in P3-R6; no scoring test failed.
- Result: **P5-R1 complete**. No protocol candidate generation, Winner selection, Grade, or Git operation was performed.
...
### Phase 1 / P1-R1: Production Project Foundation

- Status: Completed
- Date: 2026-07-15
- What was done: Created the isolated production Unity project under `app/`, the offline Python analysis package under `analysis/`, machine-readable environment contract `sc8-production-environment-v1`, full direct/transitive Python dependency pins, process-local setup/preflight/test/build PowerShell commands, root/local setup documentation, deterministic empty bootstrap scene, and a Windows x86-64 shell build. Established five production assembly boundaries: engine-independent Core and Data, engine-independent Services over Data, UI over Services, and Test Logic over Services plus Unity Input System. Configured Unity Input System package-only handling and pinned Unity 6000.5.3f1, Input System 1.19.0, Test Framework 1.7.0, Python 3.12.13, pandas 3.0.1, NumPy 2.3.5, and matplotlib 3.11.0. Added boundary tests that prohibit UI/Test Logic direct Data access and production references to the Phase 0 calibration harness. No database, profile, formula, scoring, target, capture, mode, or product UI behavior was implemented.
- Test results (per Definition of Done in AGENTS.md): Environment preflight passed every Unity/Python/package/dependency and frozen calibration-config SHA-256 pin. Production Python tests passed **6/6**; the complete Phase 0 Python regression passed **72/72**. Production Unity 6000.5.3f1 EditMode/NUnit passed **7/7**, 0 failed/skipped/inconclusive. The repeatable Windows x86-64 build command passed and produced `app/Builds/Windows/SensCalibr8.exe` (667,648 bytes) in an ignored local output directory. Layer assembly compilation, exact asmdef dependency direction, Input System package-only setting, enabled bootstrap scene, calibration hash, and calibration-harness isolation all passed. No calculation or input-validation behavior changed in this foundation-only round, so PSA and profile validation cases are not applicable; their existing Phase 0 regressions remain green.
- Issues encountered: The host blocks direct `.ps1` execution by policy, so documentation and commands use process-local `powershell -ExecutionPolicy Bypass` without altering machine policy. Windows PowerShell did not set `$LASTEXITCODE` when invoking the Unity GUI executable directly; scripts were corrected to hidden `Start-Process -Wait -PassThru` execution with explicit exit-code checks. The first Unity run passed compilation but failed one self-scanning guard because the test file contained its own forbidden assertion literal; the scanner was correctly limited to non-test production source and the clean rerun passed 7/7. The globally bundled Python runtime lacked matplotlib, so a project-local ignored `.venv` was created from Python 3.12.13 and installed from exact pins. Per the project owner's standing instruction, no Git operation was performed.
- Next step: Await explicit authorization for `Phase 1 — P1-R2: Central configuration and domain contracts`. That round will implement typed immutable loaders and domain contracts; SQLite schema/migrations remain deferred to P1-R3 and production Test Engine behavior remains deferred to Phase 3.

### Phase 1 / P1-R2: Central Configuration and Domain Contracts

- Status: Completed
- Date: 2026-07-15
- What was done: Added engine-independent Core domain contracts for test modes, protocol phases, grades, profile descriptors, formula versions, and calibration configuration versions. Added immutable production configuration/value objects and the Service-Layer `FrozenCalibrationConfigurationLoader`. It loads only the P0-R7 SHA-pinned acceptance envelope and `calibration_config_v1`, then fails closed unless the accepted/immutable identity, explicit timing-waiver limitation, complete exact 20-field database projection, finite scalar values, canonical embedded JSON, scalar/embedded contract agreements, and every frozen source hash agree. Added the accepted immutable typed `research-constants-v1` contract for the non-Phase-0 `RESEARCH.md` constants; Phase 0 scoring, geometry, signal, and protocol values remain centralized solely in `calibration_config_v1`. Added equivalent immutable Python readers plus a common parity manifest and documented the boundary. No SQLite, profile, scoring, input-capture, target, or Test Engine behavior was added.
- Test results (per Definition of Done in AGENTS.md): Production Python tests passed **11/11**, including accepted projection, immutability, mutation rejection, source hash, parity, and typed general-constant checks. Unity 6000.5.3f1 EditMode/NUnit passed **12/12**, 0 failed/skipped/inconclusive, including C# accepted projection, immutable-public-contract, status-tamper rejection even with a recomputed envelope hash, typed general constants, and Core-only domain-contract checks. The Phase 0 Python regression suite passed **72/72**; no calculation or profile-input validation behavior changed, so their worked examples and edge cases remain deferred to their owning rounds.
- Issues encountered: The first Unity run exposed an NUnit API mismatch in the new exception assertion (`Throws.InvalidDataException` is unavailable in this Unity NUnit version). The assertion was replaced with `Throws.TypeOf<InvalidDataException>()`, and the clean rerun passed. No configuration or calibration artifact was modified. Per the project owner's standing instruction, no Git operation was performed.
- Next step: Await explicit authorization for `Phase 1 — P1-R3: SQLite schema and migrations`. That round will create the documented schema/migration boundary and enforce `PRAGMA foreign_keys = ON`; repositories remain deferred to P1-R4.

### Phase 1 / P1-R3: SQLite Schema and Migrations

- Status: Completed
- Date: 2026-07-16
- What was done: Implemented a Data-layer-only SQLite native C API boundary, deterministic provisioning of the SHA-256-pinned Unity SQLite runtime, explicit native initialization, connection-level `PRAGMA foreign_keys = ON` enablement and verification, and typed error wrapping. Added ordered/checksummed transactional migrations and the complete authoritative schema: 18 product tables, the `schema_migrations` metadata table, 23 explicit query indexes, all documented uniqueness/check/`NOT NULL` constraints, and all 33 foreign keys with `ON DELETE CASCADE`. Added transactional bootstrap of the accepted exact 20-field `calibration_configs` projection, idempotent reopen behavior, and fail-closed rejection of stored configuration drift. Updated production setup/preflight/test/build automation and architecture/setup documentation. No repositories, profile workflow, scoring, target, input capture, UI, or Test Engine behavior was implemented.
- Test results (per Definition of Done in AGENTS.md): Unity 6000.5.3f1 EditMode/NUnit passed **20/20**, 0 failed/skipped/inconclusive, including **8** real temporary-file SQLite integration tests for migration metadata/versioning/checksum drift, the 23 explicit indexes, per-connection foreign-key enforcement, all 33 cascades, required/unique fields, profile cascade deletion, exact configuration bootstrap/idempotency, and stored configuration drift rejection. An independent schema parity audit compared all **18/18** `ARCHITECTURE.md` table definitions against migration 1 for column set, SQLite type mapping, `NOT NULL`, foreign-key target, and cascade behavior with **0 mismatches**. Production Python passed **11/11** and the complete Phase 0 Python regression passed **72/72**. Environment preflight passed every pin including SQLite SHA-256. Windows x86-64 build passed and packaged `sqlite3.dll` under `SensCalibr8_Data/Plugins/x86_64/` with the exact pinned SHA-256. Formula worked examples and profile validation cases are not applicable because this round adds persistence structure only and no calculation or product input behavior.
- Issues encountered: Unity's bundled managed `Mono.Data.Sqlite` candidates were not a reliable production dependency: one was a facade and another produced runtime conflicts. The implementation therefore uses the pinned native SQLite C API directly. That Unity-provided SQLite build is compiled with automatic initialization omitted, which initially caused an access violation until the boundary was corrected to call `sqlite3_initialize()` before `sqlite3_open_v2`; the clean full rerun passed. The schema audit also established that the authoritative document contains **33**, not 31, foreign-key declarations, and the test expectation was corrected without changing the schema. The native plugin and generated build outputs remain ignored local artifacts. Per the project owner's standing instruction, no Git operation was performed.
- Next step: Await explicit authorization for `Phase 1 — P1-R4: Repositories and transaction boundary`. That round will add validated repositories, mappings, aggregate transaction preservation, database error logging/user surfacing, and recovery behavior on top of this migration boundary; P1-R5 remains responsible for broader integrity/failure-path coverage.

### Phase 1 / P1-R4: Repositories and Transaction Boundary

- Status: Completed
- Date: 2026-07-16
- What was done: Added typed, Data-layer-only repositories for profiles; cycles; protocol candidates with source provenance; protocol batteries; accepted calibration-configuration identity; and completed session capture. Candidate/battery persistence verifies its profile/cycle/phase/sensitivity parent agreement before writing. Added an immediate SQLite transaction boundary that writes one session header, mandatory timing diagnostics, shot evidence, Tracking trial/window evidence, and raw mouse samples as one aggregate; any failed child write rolls back every row in that aggregate. Raw SQL methods are now internal to the Data assembly, with test-only friend visibility, so Services, UI, Test Logic, and Python cannot execute production SQL directly. Added structured `DataAccessException` failure kind/recovery action plus injected `IDataFailureReporter` handoff for future logging and UI surfacing. No UI, product profile lifecycle, calculation, scoring, adaptation finalization, target behavior, or input capture was implemented.
- Test results (per Definition of Done in AGENTS.md): Unity 6000.5.3f1 EditMode/NUnit passed **26/26**, 0 failed/skipped/inconclusive, including **6** P1-R4 repository integration tests. Verified profile mapping; candidate/source provenance; battery-parent agreement; atomic shot-capture commit; atomic Tracking trial/window commit; full rollback of session, diagnostics, and shots after a deliberately duplicated raw-sample uniqueness failure; structured `ConstraintViolation` / `PreserveInMemorySession` reporting; rollback of a candidate parent after duplicate source provenance; and Data-only raw SQL visibility. Production Python passed **11/11** and Phase 0 calibration regression passed **72/72**. No formula or user-input-validation behavior was added, so PSA worked examples and profile edge cases remain owned by Phase 2. Static source scan found no raw SQL pattern outside Data production code.
- Issues encountered: The schema has no persistent application-error log table, and adding one would change the authoritative schema beyond this repository round. The implemented reporter is therefore an injected, UI-agnostic handoff that preserves structured failure/recovery information without falsely promising database restore. Tracking persistence was initially absent from the aggregate; it was added before completion so both shot and Tracking evidence share the same atomic boundary. Per the project owner's standing instruction, no Git operation was performed.
- Next step: Await explicit authorization for `Phase 1 — P1-R5: Data-integrity verification`. That round will expand automated integrity/failure-path coverage for the remaining schema contracts, version persistence, cascade/uniqueness boundaries, and controlled database failures before Phase 1 can exit.

### Phase 1 / P1-R5: Data-Integrity Verification

- Status: Completed
- Date: 2026-07-16
- What was done: Added the final Phase 1 integrity/failure-path test layer over the P1-R3 schema and P1-R4 repositories. Verified one-mode-per-battery uniqueness and preservation of the original session after a duplicate attempt; duplicate candidate-source rollback; full profile cascade through persisted cycles, candidates, source provenance, batteries, sessions, diagnostics, shots, and raw samples; historical `formula_version` plus `calibration_config_id` persistence; foreign-key violation rejection without collateral rows; retry classification for an invalid database location; and clean SQLite `integrity_check`/`pragma_foreign_key_check` results after valid writes. No production schema, formula, or user behavior was changed.
- Test results (per Definition of Done in AGENTS.md): Unity 6000.5.3f1 EditMode/NUnit passed **32/32**, 0 failed/skipped/inconclusive, including **6** new P1-R5 integrity tests. Production Python passed **11/11** and the complete Phase 0 Python regression passed **72/72**. P1-R3/P1-R4 schema and repository coverage remains green; no raw SQL was added outside Data/test code. Formula worked examples and Phase 2 input-validation edge cases are not applicable to this persistence-only round and remain explicitly assigned to P2-R1.
...
### OQ-022: Phase 2 Winner Selection After Stabilization

- Status: Resolved — user approved recommendation on 2026-07-17
- Resolution: After every candidate stabilizes, select the unique highest mean complete-battery Performance Score. Exactly equal highest means are an explicit tie: persist no Winner/`phase_history` row and do not continue automatically into Phase 3 or emit a final Best Sensitivity. The same Winner rule applies to Phase 3 because it repeats the Phase 2 test structure.
- Specification gap: `RESEARCH.md` Section 11.2 and `FEATURES.md` Section 3.3 define candidate generation, 5-10 complete batteries per value, and per-candidate CV stabilization, then refer to a Phase 2 Winner without defining the between-candidate decision rule. CV establishes repeatability within a candidate; it does not by itself determine which stabilized candidate is best.
- Possible options: choose the stabilized candidate with the highest mean Performance Score (smallest burden, but requires an explicit exact-tie policy); run a fresh top-two confirmatory significance gate analogous to Phase 1 (stronger inference but adds 10 matched pairs not authorized by the current Phase 2 source); or carry every practically tied top candidate into a defined Phase 3 union (preserves uncertainty but requires a new tie tolerance and expansion rule).
- Recommendation: choose the highest mean Performance Score only after every candidate stabilizes, and treat exactly equal means as a statistical tie requiring an explicit user-visible decision rather than an arbitrary eDPI tie-break. This fits the existing Phase 2 workload without inventing another large confirmatory block, but still requires project-owner approval.
- Implementation constraint: Do not write Phase 2 Winner/`phase_history` or generate Phase 3 candidates until this rule and exact-tie behavior are approved.
